...
void P3ABCGate::transition(Phase phase,UiCoordinator& ui,const char* hint){phase_=phase;phaseAt_=millis();ui.setHint(hint);}
bool P3ABCGate::waitsForHuman() const {
    return (phase_==Phase::A && a_.waitingForHuman()) || phase_==Phase::Intro || phase_==Phase::Lyrics || phase_==Phase::WaitView || phase_==Phase::Cover || phase_==Phase::AudioConfirm;
}
...
void P3ABCGate::begin(UiCoordinator& ui,PlayerRuntime& player,M5GFX& display){
    ...
    writeSkipped("/ADVWalkman/logs/p3b-last.txt","waiting_p3a");
    writeSkipped("/ADVWalkman/logs/p3c-last.txt","waiting_p3a");
    ...
    ui.showLibrary();a_.begin(display.width(),display.height(),display.getRotation(),true);
    phase_=Phase::A;ui.setHint(a_.hint());lastAt_=millis();
}
bool P3ABCGate::beforeAction(UiAction action,const RawKeyEvent& raw,UiCoordinator& ui,PlayerRuntime& player){
    ...
    if(phase_==Phase::Intro && action==UiAction::Confirm){
        ...
        ui.handleAction(action);transition(Phase::Cover,ui,"COVER OK?\nENTER / ESC NO");
...
void P3ABCGate::service(UiCoordinator& ui,PlayerRuntime& player){
    ...
    const bool realMediaConfirmation=phase_==Phase::Intro || phase_==Phase::Lyrics || phase_==Phase::WaitView || phase_==Phase::Cover;
    if(realMediaConfirmation && (media.lyrics==MediaState::Missing || media.lyrics==MediaState::Error || media.cover==MediaState::Missing || media.cover==MediaState::Error)){
        fail("real_track_media_missing_or_bad",ui,player);return;
    }
    if(phase_==Phase::A){
        ...
        if(a_.finished()){
            ...
            cRan_=true;transition(Phase::PrepareIntro,ui,"SCRIPT SEEK\nINTRO AT 0");
        }return;
    }
    switch(phase_){
    case Phase::PrepareIntro:
        ...
        frameBaseline_=media.frames;transition(Phase::Intro,ui,"INTRO + HEADER\nENTER / ESC NO");break;
    ...
    case Phase::SeekCheck:
        if(pulse_==0){
            if(now-phaseAt_>=4500){player.pause();pausedPosition_=player.snapshot().positionMs;transition(Phase::Lyrics,ui,"LYRICS / LATIN\nENTER / ESC NO");}
    ...
    case Phase::Warmup:
        if(snapshot.state==PlayerState::Playing && snapshot.sampleRateHz==44100 && snapshot.pcmBuffersSinceReset-bufferAt_>=10){
            player.resetDiagnostics();b_.begin(benchmark_.path,now);bRan_=true;
            ...
            frameBaseline_=media.frames;readBaseline_=media.reads;heapAt_=ESP.getFreeHeap();windowAt_=now;pulse_=0;
    ...
    case Phase::Measure:{
        ...
        if(media.layoutError || media.invalidUtf8 || ui.fonts().stats().missing || ui.fonts().stats().ioErrors || ui.fonts().stats().drawMisses){fail("p3c_resource_or_layout",ui,player);break;}
        ...
        if(now-windowAt_>=60000){
            if(measuredFrames_<6 || measuredReads_==0 || media.lyrics!=MediaState::Ready || media.cover!=MediaState::Ready){fail("media_not_exercised",ui,player);break;}
    ...
    case Phase::PauseCheck:
        if(now-phaseAt_>=3000){pauseOk_=snapshot.state==PlayerState::Paused && snapshot.positionMs==pausedPosition_ && media.page==pausedPage_;
            if(!pauseOk_){fail("pause_lyric_sync",ui,player);break;}transition(Phase::AudioConfirm,ui,"SOUND/OVERLAY OK\nENTER / ESC NO");}break;
...
void P3ABCGate::fail(const char* reason,UiCoordinator& ui,PlayerRuntime& player){
    reason_=reason;player.pause();phase_=Phase::Failed;resultDrawn_=false;
    if(!restoredRun_){
        if(bRan_)b_.writeLog(SD,model_,drawing_,overlay_,displayOk_,coverOk_);else writeSkipped("/ADVWalkman/logs/p3b-last.txt",reason);
        if(cRan_)writeCLog("FAIL",ui,player);else writeSkipped("/ADVWalkman/logs/p3c-last.txt",reason);
...
bool P3ABCGate::writeCLog(const char* result,UiCoordinator& ui,PlayerRuntime& player){
    ...
    f.printf("result=%s\nversion=%s\ntask_executed=1\nprimary_failure=%s\nautomatic_ms=%lu\n",result,ADV_WALKMAN_VERSION,reason_,(unsigned long)autoMs_);
    ...
    f.printf("minimum_heap=%lu\nmedia_budget_bytes=%u\nlyrics_layout_error=%d\nlyrics_invalid_utf8=%d\n",(unsigned long)minimumHeap_,unsigned(kMediaBudgetBytes),media_.layoutError,media_.invalidUtf8);
    f.flush();
...
bool P3ABCGate::renderResult(M5GFX& display){
    ...
    if(!resultDrawn_){
        ...
        UiTextLayout::draw(display,phase_==Phase::Passed?"ENTER: LIBRARY\nPaused. No auto play.":reason_,{6,90,123,120,6,4,true});resultDrawn_=true;
    }return true;
}
} }
