...
0.8.2已完成71项PC检查、编译期回归、六环境构建与23文件SD交付核验；旧benchmark仅精确清理5个受管文件，PC可恢复，存档/历史日志未变。编译与PC参考不能证明设备时序达标，A/B/C/D保持DEVICE TEST。

仍是一次安装同名P3ABC BIN、自由操作后台记录，不恢复顺序Gate。重点检验单次View、准备时连按、播放/暂停Tab往返、连续浏览/翻页/Esc、4首问题歌曲及至少60秒连续播放。T保存后可手动重启；新日志按版本/启动编号核验。0.8.1峰值继续保留，不预告性能已通过。

以下0.8.1描述为历史实现，不代表相应问题已经真机解决。
...
分别记录 A/B/C/D 主路径覆盖；连续播放 60 秒是被动覆盖项，不是固定测试阶段。
...
| P3D | P3-07 视觉完成、P3-10、P3-11 | 黑胶曲库、独立曲库封面、设置、息屏 Soft Lock、最终 UI 校准 | 同一BIN，与P3C修复合并自由验收 |

执行顺序固定为 `P3A → P3B → P3C → P3D`，不合并代码职责。用户已批准将尚未真机
验证的P3C导航修复与P3D合并实操；此前“A-fix+B+C之后另装Gate D”的安排不再适用。
下面旧版本记录仅保留历史事实，不覆盖当前自由验收流程及未通过项。
...
`0.8.0-p3d.ui`：独立普通彩色120×120曲库封面（AveMujica官方动画五人合图），两行
完整名称、按实际字宽的弧形短名，最多三张黑胶，切换约160ms/最多4帧，平时静止。
四项中文设置，亮度70%/步长10%，独立A/B显示存档；Player3分钟/其他30秒，唤醒无5秒
特殊时限。完整物理位图先吞掉唤醒整组键，全部松开后才派发。返回Launcher先保存、
运行时选择有效TEST App，不改Launcher设置或分区。实现/自动检查/交付以
`P3D_IMPLEMENTATION.md`为准；P3D不增加实体上一首/下一首和DSP。
