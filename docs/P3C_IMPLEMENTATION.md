# P3C media contract — 0.7.5-p3c.closure
...
不是直接缩小JPG。兼容旧26×20、30×24、34×26，新增40×32（默认）及48×40，
120×144 canvas不变。使用95个可打印ASCII候选，匹配形状、密度、边缘及颜色，
轻量局部对比度/锐化；输出字符表与原尺寸/4倍预览，不随机混字。
...
字体编号明确区分CJK12/14/16/18与Latin10/12/14。PC以4倍分辨率栅格化后缩小，
18 px楷体轻量加粗。SD仍8-bit coverage；RAM压为4-bit，15 KiB变长arena按
ceil(width×height/2)存储。200项16-byte metric加3×256-byte Latin宽度/advance
紧凑表共3968 bytes，不超过4 KiB。空格零面积
...
预取，显示结束再解除pin。密集Latin每页最多300 glyph，
使用 8-byte 紧凑位置记录。离开 Player 释放
歌词工作集和资源句柄，字体缓存供其他页面使用；不可见动画不推进。

横排UI使用CachedUiFont的实际advance、基线与边界；正常显示不使用Font0 ASCII。
字体未绑定等故障才用备用字体。只预取可见窗口，避免长Title超过缓存容量时反复逐出。
歌词采用实际Latin14 advance/bitmap extent与CJK18 cell；整词高度同时包含旋转后
字形宽度，避免advance略窄时越过列底。先整帧准备字模，
...
Missing。完整呈现期优先完成18px条带，UI burst最多64步 / 6ms软预算；每个burst前
先服务音频。资源worker与绘制分轮执行，避免同次UI调用堆叠读取和绘制。
...
英文整词按实际advance/bitmap extent预量：剩余列高不足则换列，只有词长超过202px才允许拆分。
...
PlayerRuntime统一用VolumePolicy映射raw=(level×102+127)/255，启动逻辑80 / raw32；先真实调整
...
记录采用6KiB固定缓冲，512B分步append；避开完整歌词呈现和Player checkpoint写入。
每15秒 / T生成带sequence、CRC32及END行的checkpoint；半截尾部不会覆盖之前完整证据。
日志 `/ADVWalkman/logs/p3-free-last.txt` 跨启动追加，boot_id从有界尾部读取后递增；
旧p3a/b/c日志不覆盖。Host按版本/启动编号区分历史失败、保留本版各启动真实异常。
T先请求Player checkpoint，等persistence idle且保存成功后写日志并显示LOG SAVED。
实际SD写入负载也在
...
启动在未开Decoder时检查真实字体两行布局及媒体定位/布局/取消；这不是播放中Seek验收。
额外记录有效View、Header可见性、字体、无歌词View无动作、跨歌偏好、启动歌曲/位置/偏好、
静音观察时长，以及音频/Library/Input/UI/字体/日志耗时。Host比较手动保存与下次启动，
要求恢复暂停、静音观察≥3秒、歌曲/偏好一致、位置差≤10秒。未覆盖项不伪造PASS。
旧Gate脚本Seek / 重启不自动执行；播放中Seek明确not_exercised，任务保持待验。

显示常量：Cover为Header28、Content188、Footer24；Lyrics无Header，Content216，
Footer24。歌词内框123×202、上6下8、CJK18微加粗/Latin14、列距1，
双语间距6、最多六列。仅当前双语组，无前后句预览；英文整词换列，过长才拆字。
保留Crucifix既有中文；9份新译文按原时间戳绑定，纯咒语不重复中文栏。小标点右上，括号 / 引号旋转。
