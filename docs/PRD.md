...
0～255；实际 M5.Speaker 映射到 0～63，新版 100% 不超过旧版约 25% 的档位。
启动显示 50%（实际 raw32），不恢复旧高音量。这是针对当前耳机的用户校准，
...
内框约 123×174 px（左右 6 px、上 6 / 下 8 px）；CJK 16 px、列间距 2 px、双语块间距
...
Pause 冻结歌词位置和分页，Header 标题仍可滚动；Seek 直接定位，不跨句补动画。
