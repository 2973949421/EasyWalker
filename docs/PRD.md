...
P5A+B以`0.10.2-p5.monochrome`实现Original、Tape、Radio、Vocal Clear四个互斥固定音效。参数不在设备端开放；播放器页9～12号键直接选择，Footer以纯黑白圆框内的O/T/R/V区分，最后成功保存的音效随Session恢复。0.10.1增强Tape与Vocal Clear的可辨识度，0.10.2移除彩色填充并隔离图标与时间／进度样式，同时保持保护器和20ms切换淡化。P5只承诺耳机路径，扬声器破音与P3/P4既有性能问题继续留在P6。
...
`Play Mode` 按 `Normal（列表单次） → Repeat One（单曲循环） → Repeat All（列表循环） → Shuffle（随机循环） → Normal` 循环。双 Play/Pause 是刻意扩大最高频功能的盲操命中区。这里的布局按物理位置冻结，不以键帽字符或旧数字快捷键表达。

手动 Previous / Next 与自然播完是两套规则：顺序类模式在首尾手动导航时循环，Repeat One 只控制自然播完，不能吃掉手动切歌；Shuffle 的 Next 沿随机顺序推进、Previous 优先按真实随机历史返回，没有历史时也必须选择一个随机邻项而非无响应。自然播完时仅 Normal 停止，Repeat One 重播本曲，Repeat All 回到列表首项，Shuffle 开始下一轮随机顺序。
...
- 进度条
- 播放 / 暂停 / 停止 / 错误图标
- 时间右侧两个约12px小标识：统一方形循环箭头，列表循环无内字、单曲循环为`1`、随机循环为`R`、列表单次为`S`；第二枚圆内直线表示Original原声
...
曲库图片资源统一为135×154；顶部显示舞台固定为174px，图片不拉伸，剩余20px由页面背景承接。任何新曲库都必须经过同一尺寸检查，不能为单个曲库另设135×173等例外。完整名称采用华文行楷18px / Kunstler Script 22px，固定在y174、高22px单行区域；短名居中，长名沿用Player的开头停5秒、24px/s左移、末尾停5秒循环，最多20次刷新/秒。只刷新名称，不挤压固定y196至屏幕底部的轮盘。四套曲库字体在4倍栅格向各侧扩1px再缩小，其余字体不变。
