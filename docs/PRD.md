...
其余 3×4 控制与 DSP 仍由 P4 实施。音量每次按下变化 8/255，饱和限制在 0～255。
...
多列，保持字号；当前完整组居中高亮，只有余量足够完整列时才显示暗色前后组。
内框约 123×174 px（左右 6 px、上 6 / 下 8 px）；CJK 14 px、列间距 2 px、双语块间距
6 px，最多七列。前奏也完整展开首组双语，不能退回只显示十个字的一列。不得裁出半字。

只有整句仍放不下才自动阅读分页，不增加翻页键。短语言保持显示，长语言续页；
双方均超长则同步推进。分页按本句到下一时间戳的区间分配，末页停至下一句，
不宣称逐字演唱同步。前奏只显示暗色首句预览，不显示“前奏”文字，不自动切 Cover。
