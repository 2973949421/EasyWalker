...
- [x] 本地安全标签及精确联合BIN/尺寸/Hash恢复副本；不做全SD镜像
- [x] 页面/请求/资源Token、Library完整帧合同、旧结果拒绝和唯一条带所有权
- [x] 旧ready条带不阻塞新浏览模型；2秒无进展一次局部恢复，二次失败仍可导航
- [x] Library/Playlist控制器及显式Scheduler；六行窗口和页面专属字体租约跨暖往返保留
- [x] 三项本次启动封面CRC验证缓存；不改变LCOV、媒体绑定或图片RAM预算
- [x] 每主循环一次官方键盘更新；固定事件队列、短按/组合键/唤醒规则不变
- [x] SaveTicket联合Player/Display/诊断事务；重复T尾随、全页面反馈、关闭回读后成功、10秒终态
- [x] 16KiB日志改1KiB流式/512byte步骤/滚动CRC；Stage A 15秒摘要、T/错误/恢复完整快照
- [x] 旧卡死顺序、1000次切库、精确Token、尾随保存/超时和日志验收边界自动回归
