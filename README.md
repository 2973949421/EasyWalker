...
> 项目名：EasyWalker
> 历史开发代号：ADV Walkman
> 当前阶段：P3仍为`FROZEN / UNVERIFIED / KNOWN ISSUES`；P5A+B音效修订固件`0.10.2-p5.monochrome`已完成本地实现并进入DEVICE TEST
> 平台：M5Stack Cardputer ADV

## 1. 项目是什么

P3冻结证据见[P3交付路线](docs/P3_DELIVERY.md)，不生成原计划的0.8.6，也不把未验证项目写成通过。P4A+B基线见[P4AB控制](docs/P4AB_CONTROLS.md)，0.9.1保存、列表、歌词与KINO资源修复见[P4A+B修复记录](docs/P4AB_FIX.md)，0.9.2边界导航、模式图标与曲库图片尺寸修复见[P4A+B Transport修复](docs/P4AB_TRANSPORT_FIX.md)。P5在播放器页启用9～12号键直接选择Original、Tape、Radio和Vocal Clear；0.10.1加强Tape/Vocal差异，0.10.2恢复纯黑白Footer并修复图标颜色泄漏。完整参数、RAM边界与验收状态见[P5音效记录](docs/P5_SOUND.md)；音频Backend、3×1536缓冲和Session v1长度保持不变。
...
仅安装同名`ADV-Walkman-P3ABC-Gate.bin`。自由检查：11首选中/未选中标题一致；Tab/View/返回；Lyrics、Cover、Playlist分别设置15秒息屏后唤醒两次；约20分钟正常耳机听歌后再返回列表与唤醒。T保存后手动重启检查暂停恢复。设置与Launcher仍独立验收。构建、资源核验和SD交付结果见[P3_REFINEMENT.md](docs/P3_REFINEMENT.md)。

### 历史入口 — 0.8.2 性能与可靠切换修复
...
普通UI使用楷体与Times，歌名14、歌手12、底栏时间14 px；曲库名称/短名改用华文行楷与Kunstler，透明音量条不加黑底。
...
暗黑天国故意无歌词，用于Cover-only验证。PC原文件及日文原稿不改；SD旧benchmark已在0.8.2退役，Crucifix X已正式入库。
...
- 看曲库五人合图、三张各带短名的黑胶及单行长名称滚动；标题不挤压轮盘，浏览时音乐持续。
